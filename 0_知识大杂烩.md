...
这个文件是最外层的知识收件箱和历史知识总汇。

平时看到想记的东西，先写在下面的“快速写入区”；后续如果内容变多，再整理到对应专题目录。
...
┌─────────────┬──────────┬──────────┬──────────┬──────────┐
│             │ SysV信号量│ POSIX有名│ pthread  │ 文件锁   │
│             │ semget   │ sem_open │ mutex+shm│ flock    │
├─────────────┼──────────┼──────────┼──────────┼──────────┤
│ 跨进程      │✅ key   │ ✅ 名字  │ ⚠️ 需shm │ ✅ 文件  │
│ 防崩溃死锁  │ ✅ UNDO  │ ❌       │ ❌       │ ✅ 自动  │
│ 超时等待    │ ✅       │ ✅       │ ✅       │ ⚠️ 非阻塞│
│ 计数信号量  │ ✅       │ ✅       │ ❌       │ ❌       │
│ 实现复杂度  │ 中       │ 低       │ 高       │ 最低     │
...
│   ├── 同进程内线程间
│   │   ├── 简单互斥 → pthread_mutex
│   │   ├── 等待/通知 → pthread_cond + pthread_mutex
│   │   └── 简单计数 → sem_init（无名POSIX信号量）
│   │
│   └── 跨进程
│       ├── 互斥访问硬件(I2C等) → System V 信号量（SEM_UNDO防死锁）✅ 你的ENVmonitor
│       ├── 防程序重复启动 → 文件锁 flock ✅ 你的ENVmonitor也用了
│       ├── 简单跨进程互斥 → POSIX有名信号量 sem_open
│       └── 复杂跨进程同步 → pthread_mutex + 共享内存
│
└── 内核态│
    ├── 保护单个整数 → 原子操作
    │
    ├── 临界区极短 + 可能在中断中 → 自旋锁
    │   ├── 同CPU中断会访问 → spin_lock_irqsave
    │   └── 只有软中断 → spin_lock_bh
    │
    ├── 临界区可睡眠（进程上下文）
    │   ├── 纯互斥 → mutex（首选）
    │   ├── 读多写少 → rw_semaphore
    │   └──需要计数 / 跨上下文释放 → semaphore
    │
    ├── 事件通知（不是互斥） → completion
    │
    └── 读极多写极少 → RCU（读者零开销）
...
变体	禁中断？	适用场景
spin_lock()	否	只在进程上下文使用
spin_lock_irqsave()	是	进程和中断都可能访问同一资源

### 2026-06-02

唯一的区分就是地址宽度
EEPROM容量	地址数量	地址宽度	例子
128B (AT24C01)	128个	8位 (0x00~0x7F)	小容量，1字节地址够
256B (AT24C02)	256个	8位 (0x00~0xFF)	1字节地址刚好
2KB (AT24C16)	2048个	16位 (0x0000~0x07FF)	1字节放不下，要2字节
16KB (背板)	16384个	16位 (0x0000~0x3FFF)	要2字节地址
一句话总结：地址越大的器件，地址位越宽；但每个地址

总结
层次	代表文件	注册什么	核心结构	作用
控制器驱动	busses/i2c-designware-*	i2c_adapter	i2c_algorithm	操作硬件发I2C信号
I2C核心	i2c-core-base.c	i2c_bus_type	总线匹配逻辑	胶水层，连接上下
设备驱动	散布各子系统	i2c_driver	i2c_client	驱动具体从设备
用户接口	i2c-dev.c	cdev	file_operations	用户态直通总线
i2c-designware是"腿"，负责走；i2c-core是"路"，负责连；i2c-dev是"门"，让用户能进来。 你的ENVmonitor就是从"门"进去，走"路"，用"腿"到达硬件。

cat /proc/devices

可以看到设备号

int init_cmd_dev(void)
{
	alloc_chrdev_region(&sata_cmd_devno, minor, DEV_NUM, "sata_cmd_dev");  // 1. 分配设备号
	cdev_init(&sata_cmd_dev, &sata_cmd_ops);   // 2. 初始化cdev
	cdev_add(&sata_cmd_dev, sata_cmd_devno, DEV_NUM);  // 3. 添加cdev
	sata_cmd_cdev_class = class_create(THIS_MODULE, "sata_cmd_cdev_class");  // 4. 创建类
	sata_cmd_cdev_device = device_create(sata_cmd_cdev_class, NULL, sata_cmd_devno, NULL, "sata_cmd_dev");  // 5. 创建设备节点
}

字符设备是一个大筐，只要不属于块设备和网络设备的，全归字符设备：

概念	比喻
open("/dev/sda")	拿到服务台的对讲机
ioctl(fd, CMD, &data)	通过对讲机下达特定指令
正常 write(fd, data)	让服务台帮你存东西（文件系统帮你管）
ioctl(fd, SG_IO, ...)	直接对仓库管理员下指令（绕过文件系统）

- 应用程序: write(fd, "hello", 5)
  ↓
  VFS:    找到fd对应的inode和file_operations
  ↓
  ext4:   ext4_file_write_iter()  ← 文件系统的write
  ↓    1. 分配数据块（块位图查找空闲块）
  │    2. 更新inode（记录文件大小、修改时间）
  │    3. 更新目录项（如果是新文件）
  │    4. 写入日志（jbd2日志，保证崩溃一致性）
- ↓
  页缓存:  把数据写入page cache（内存），标记为dirty
  ↓ （不是立刻写盘！）
  回写线程: pdflush/kworker周期性把dirty页写回磁盘
  ↓
  submit_bio() → 通用块层 → SCSI/SATA/NVMe驱动 → 磁盘
- 
- 
- 查看cpu使用率这些
...
- 想快速找旧知识，直接在本文件里 `Ctrl+F` 搜关键词。
- 想系统复习，可以去 `0.基础知识` 看按主题拆分后的版本。
- 面试、简历、岗位 JD 相关内容优先放到 `1.面试资料`。

## 历史知识汇总

下面内容来自之前整理过的基础知识笔记，保留在这里是为了方便统一搜索。

### Linux 常用命令

> 来源：`0.基础知识/01_基础命令与脚本/01_Linux常用命令.md`
...
> 来源：`0.基础知识/01_基础命令与脚本/02_Shell脚本常用写法.md`
...
10) echo 123456|passwd --stdin stu01 给一个用户设置密码
11) 创建一个组 groupadd class 创建一个用户useradd u1p
12) 往一个组里添加用户 useradd -G class u1 往clas这个组里添加u1这个用户
13) 查看用户组 cat /etc/group 查看用户 cat /etc/passwd
14) for i in {2..22} 写法正确，for i in{2..$val} 写法错误，因为for中只能指定确定的数值
15) touch file{5..8} 可以创建file5到file8之间包括自身的所有文件
16) [ -s file5 ]  ||  echo hby 在终端可以这样使用
17) if [ $(($i%2)) -eq 0 ]; then
    $[$i%2] 引用变量必须这样写
    注：if后面有空格，中括号两边有空格
    $() 和效果一样
18) for i in $(seq 2 $[$val-1]) 等价于 for i in `seq 2 $[$val-1]`
19) if 和elif后面需空格
20) echo 1 >> 1.txt 会直接创建1.txt文件
21) 记得if[  ] 判断的时候只有一个等号
22) [ !-d ./dir1 ]; 判断一个文件不是目录
23) [ file -ef file2 ];echo $? 比较两个文件是否一致，也就是他们的id号是否一致，内容一致id号也会不一致
24) let n=n**3 计算一个数的幂值
25) echo
    ...
    返回登录shell的PID
26) ln -s file1 test1   if[ -L ./test1 ];echo $? 0 判断是不是一个链接文件
27) test -r i2cdetect;echo $? 判断icdetect文件是不是可读的l
28) -s是判断文件是否不为空，！-s判断文件是否为空文件，要和字符串的-z     和-n区分开
29) test -z "hello";echo $? 前面为空就会打印0,echo &?假就是1所以打印的是1，-z 就是空就是1，非空就是0，所以test –z “hello” 执行的结果就是0，但是echo $?结果0打印就是1
30) echo $(( 256*22 ))等价于echo $[ 256*22 ]
31) -e	判断文件是否存在（link文件指向的也必须存在）	exists
32) -f	判断文件是否存在并且是一个普通文件	file
33) [ -d  ./file1 ];echo $? 如果是目录文件在就会打印0，但是是文件的话就打印别的数字
34) ll -i 可以显示inode号
35) $0 当前执行脚本的名字，$1 当前执行脚本的第一个参数一次类推，$* 脚本输入的所有的参数和$@一样
36) echo $?查看上一次的shell命令有无执行成功，成功的话就会显示0，没有成功就会显示别的数字
37) ccc=itcast export ccc 等价于export ccc=itcast 和declare -x 定义一个环境变量和export一样
38) /etc/profile 全局环境变量信息
39) env 查看当前用户的环境变量
40) declare -r B=111  定义B只读
41) 双引号可以引用变量
42) read -p "Input": ip < ip.txt 把ip.txt的内容当作密码输入给ip
43) B=$(uname -r)等同于b=`uname -r`
44) echo ${A:2:3} 截取第2个字符的后面三个
45) read -t 3 -p "input": name 限制输入三秒
46) read -n 5 -p "INput your name": name 限制输入个数
47) read -s -p "Input your name  " pass 隐藏密码
48) read -p "Input your name" name 输出相应提示
49) unset取消变量
50) bash -x 25.sh 查看脚本调试过程
51) bash -n  查看脚本语法有误错误
52) cat phonenum.txt |sort -u|wc –l  sort -u 的作用是剔除相同行，wc -l是计算行数
53) grep -v 就是反向查找，查找不含后续字段的行
54) grep -w 就是精准查找，默认只匹配一个单词
55) shell中if[-z]就是后面是数据是空的它就是真的，就成立
56) awk -F 后面加一个.就是指定了分隔符
57) awk {print $2} 就是一行行读取文件，以空格为分割符，打印第二个字段
58) modprobe命令用于智能地向内核中加载模块或者从内核中移除模块
59) depmod -a
    这个命令一般放在make modules_install之后。depmod可检测模块的相依性，供modprobe在安装模块时使用。
60) ggYG复制全部
61) 数组有两种定义的方式一种是单个单个定义
    array[0]=hby
    array[1]=harry
    还有一种是整体定义
    arry={5 6 7}
...
62) source和./执行脚本的区别？？？
63) grep -s 选项表示不显示不存在或无匹配文本的错误信息
64) sed -n 4p打印第四行
...
> 来源：`0.基础知识/02_C语言与基础概念/00_C语言知识_信号量.md`
...
> 来源：`0.基础知识/02_C语言与基础概念/01_C语言基础.md`
...
2) BIT(3) 是一个宏，用于设置一个二进制位，具体的实现可能类似于 (1 << 3)。在这种情况下，BIT(3) 表示将第 3 位设置为 1。
3) cpunum_temp=strtoul(tmp, NULL, 10);把tmp按10进制转化输出
4) 代码怎么变成一个可执行文件的，先编译出所有目标文件，然后把这些目标文件链接起来首先预处理（头文件的包含，注释的删除，define的替换）编译（将.c代码翻译成汇编代码）汇编（将汇编代码转化成二进制代码）合并段表就成了可执行文件
5) GBK编码，一个汉字占两个字节。 UTF-8编码是变长编码，通常汉字占三个字节
6) 1、全局变量（所有文件可见的）  因此extern可以获取2、静态全局变量（当前文件可见的）
7) static作用？？
   分两种情况
...
8) extern g_para_t envpara 类似这种，就是声明了一个外部的变量，extern只能用于声明全局变量
...
11) 正数在计算机中存储的是原码，而负数在计算机中存储的是补码
12) 无符号数据类型（%u）
13) new / delete 是C++运算符，malloc / free是C/C++语言标准库函数
    18）把整型16转换为8进制，存在str里面，存在str里面的是字符（其中16可改为其他想要转换的整型数字，如25，40...），（8可改为其他想要转换的进制，如2，4，16...）itoa(16,str,8)
14) register这个关键字请求编译器尽可能的将变量存在CPU内部寄存器中，而不是通过内存寻址访问，以提高效率。注意是尽可能，不是绝对。因为，如果定义了很多register变量，可能会超过CPU的寄存器个数，超过容量。
15) int arr[4]={1,2,3,2};int len_arr = sizeof(arr);---------->len_arr等于16
...
> 来源：`0.基础知识/03_Linux内核与调试/01_Linux_GDB_内核基础.md`
...
> 来源：`0.基础知识/04_PCIe与硬件/01_PCIe基础_BAR_MSI.md`
...
> 来源：`0.基础知识/04_PCIe与硬件/02_I2C与硬件时序.md`
...
> 来源：`0.基础知识/05_工具词汇与杂项/01_专业词汇与工具.md`
...
> 来源：`0.基础知识/05_工具词汇与杂项/02_每日学习总结.md`
...
> 来源：`0.基础知识/05_工具词汇与杂项/03_技术随手记录_2024.md`
...
逻辑卷：动态智能管理空间
当你听到"LVM"时，就想象那个可以随时调整大小的智能图书馆，是不是感觉清晰多了
...
不同内核传参方式对比
为了帮助您更好地理解何时该使用哪种方式，这里有一个简单的对比：

使用场景	操作方法	特点
启动时自动加载/系统级固定配置	在 /etc/modprobe.d/ 下配置 .conf 文件	持久化，配置一次后每次启动自动生效；便于集中管理和自动化部署。
临时测试或手动调试	使用 insmod 或 modprobe 在命令行直接指定	灵活快捷，但每次加载都需要重新输入命令。
运行时临时修改	通过 sysfs 接口（如 echo value > /sys/module/module_name/parameters/param_name）	无需重新加载模块即可动态调整部分参数，但非持久化，重启后失效。

cat i2c-designware-core.conf
options i2c-designware-core pesw_sda_hold_time=5

module_param(scu_disk_slot,int,S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(scu_disk_slot, "scu disk slot(default=-1)");
cat /sys/module/sata_drv_interface/parameters/scu_disk_slot
...
1. PCIe 基础：枚举、配置空间、BAR、MSI/MSI-X、DMA
2. PCIe 设备驱动模板
3. NVMe 驱动核心流程
4. blk-mq / queue / doorbell / completion
5. PCIe 总线驱动、RC、资源分配、MSI domain
...
您好，考虑之前是浙江宇视下面的x86存储服务器部门做研发的投递此岗位嘛

你玩手机的时候人家拿起了《人性的弱点》《销售铁军》《销售的革命》《信任五环》《高绩效销售》《影响力》《华尔街之狼》《新解决方案销售》《市场营销学》《每天学点销售礼仪》《销售中的心理学》《成交高于一切》《华为执行力》《销售的金钥匙》……
