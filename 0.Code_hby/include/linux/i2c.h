/*
 * 简化版 i2c.h（用于仓库注释说明）
 * 仅包含 `struct i2c_driver` 的定义与成员注释，便于源码阅读。
 * 注：此文件为说明性复写，不为完整内核实现。
 */
...
/*
 * struct i2c_driver
 * 这是驱动在 I2C 子系统中注册时使用的结构体。下面的成员是常见字段，
 * 注释以中文说明其典型用途与参数含义，方便阅读与维护。
 */
struct i2c_driver {
    /*
     * probe:
     *   当 I2C 总线上出现匹配设备时由核心调用。
     *   典型签名：int (*probe)(struct i2c_client *client, const struct i2c_device_id *id)
     *   任务：初始化设备、分配驱动数据、注册 sysfs/子设备等。
     *   返回 0 表示成功；非 0 表示 probe 失败并取消绑定。
     */
    int (*probe)(struct i2c_client *client, const struct i2c_device_id *id);

    /*
     * remove:
     *   驱动与设备解绑时调用，用于释放 probe 中分配的资源并撤销注册。
     *   典型签名：int (*remove)(struct i2c_client *client)
     */
    int (*remove)(struct i2c_client *client);

    /*
     * shutdown:
     *   在系统关机或重启阶段调用，用于把设备置于安全状态或断电之前的清理。
     *   签名：void (*shutdown)(struct i2c_client *client)
     */
    void (*shutdown)(struct i2c_client *client);

    /* 电源管理相关回调（可选，内核版本/配置不同） */
    int (*suspend)(struct i2c_client *client, void *state);
    int (*resume)(struct i2c_client *client);

    /*
     * id_table:
     *   指向 struct i2c_device_id 数组的指针，用于基于 id 的匹配（legacy 匹配机制）。
     *   当使用非设备树/ACPI 的平台驱动时，常见于驱动内声明一个 id 列表供匹配使用。
     */
    const struct i2c_device_id *id_table;

    /*
     * of_match_table:
     *   指向设备树匹配表（struct of_device_id）的指针，用于基于设备树的匹配。
     */
    const struct of_device_id *of_match_table;

    /*
     * acpi_match_table:
     *   指向 ACPI 匹配表（struct acpi_device_id）的指针，用于基于 ACPI 的匹配。
     */
    const struct acpi_device_id *acpi_match_table;

    /*
     * driver:
     *   嵌入的通用 `struct device_driver`，包含驱动名、owner、pm 等与设备模型交互的元数据。
     *   通过该成员驱动可与通用设备模型连接并使用通用接口。
     */
    struct device_driver driver;

    /*
     * owner:
     *   模块所有者（通常由宏设置为 THIS_MODULE），用于模块引用计数，防止模块在使用中被卸载。
     */
    void *owner;

    /*
     * address_list / detect:
     *   旧式、较少使用的字段：用于在没有固件/DT 信息的平台上扫描地址并侦测设备。
     *   现代驱动通常依赖设备树/ACPI/平台数据而不是主动扫描。
     */
    const unsigned short *address_list;
    int (*detect)(struct i2c_client *client, unsigned short id);

    /*
     * class:
     *   驱动/设备分类标志（例如 I2C_CLASS_HWMON），用于按照功能对设备分组或过滤匹配。
     */
    unsigned int class;
};
